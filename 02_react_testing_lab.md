...
The start code provided has a Calculator model with methods to add, subtract, divide, and multiply given numbers to a total (`src/App.js`). The Calculator model has properties to keep track of calculations as it performs them. There is also a UI model to display the buttons and trigger click events (`src/components/Calculator.js`)
...
Your task is to complete both unit and integration testing on the App and Calculator models and the UI. You will be using the React testing library to complete the tests on the Calculator model, and another testing framework - Cypress - to complete the UI tests.
...
- To run the UI integration tests with Cypress:
    ...
    3. Run the `App.spec.js` file in Cypress to run the integration tests.
